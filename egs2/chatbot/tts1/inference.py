...
train_config="/home/espnet/egs2/chatbot/tts1/exp/tts_finetune_fastpeech2/config.yaml",
model_file="/home/espnet/egs2/chatbot/tts1/exp/tts_finetune_fastpeech2/valid.loss.ave_5best.pth",
vocoder_tag="parallel_wavegan/ljspeech_parallel_wavegan.v1",
device="cuda",
 )
model_dir= "/home/espnet/egs2/chatbot/tts1/exp/tts_finetune_fastpeech2"
spembs = None
...
def tts(text, preprocess = True):
    if preprocess:
        text = frontend(text)
    with torch.no_grad():
        start = time.time()
        wav = text2speech(text,speech=speech, spembs=spembs,decode_conf={"alpha": 1.2 })["wav"]
...
if __name__ == '__main__':
    ...
    tts(text)
    # rtfs = []
    # with open("/home/espnet/egs2/chatbot/tts1/data/test_phn/text","r") as t:
    #     for line in t.readlines():
    #         id, text = line.split(" ")[0], " ".join(line.split(" ")[1:]).strip('\n')
    #         rtfs.append(tts(text, False))
    # print(len(rtfs),np.array(rtfs).mean())
